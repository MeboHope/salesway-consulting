...
import {
  TrendingUp, Target, Megaphone, Users, Settings, GraduationCap,
  CheckCircle2, ArrowRight, Phone, Search, Lightbulb, Rocket, LineChart,
} from 'lucide-react';
import { Button } from '@/components/ui/button';
import { Card, CardContent, CardHeader, CardTitle, CardDescription } from '@/components/ui/card';
import { Badge } from '@/components/ui/badge';
import { services, processSteps, type Service } from '@/lib/data';
...
const iconMap: Record<string, React.ComponentType<{ className?: string }>> = {
  TrendingUp, Target, Megaphone, Users, Settings, GraduationCap,
  Phone, Search, Lightbulb, Rocket, LineChart,
};
  ...
  useEffect(() => {
    (async () => {
      const { data, error } = await supabase
        .from('services')
        .select('title, slug, summary, details, icon, features')
      ...
      <section className="relative py-20 sm:py-28">
        ...
        <div className="relative mx-auto max-w-4xl px-4 text-center sm:px-6 lg:px-8">
          ...
          <p className="mt-6 text-lg text-muted-foreground text-pretty">
            Every engagement is tailored to your business, industry, and stage
            of growth. No templates, no fluff — just practical strategies and
            hands-on implementation that delivers real results.
          </p>
            ...
            {serviceList.map((service, i) => {
              ...
              return (
                <div
                  key={service.slug}
                  id={service.slug}
                  className={`grid gap-8 lg:grid-cols-2 lg:items-center reveal ${isReversed ? 'lg:[direction:rtl]' : ''}`}
                >
                  <div className={`space-y-6 lg:[direction:ltr] ${isReversed ? 'lg:order-2' : ''}`}>
                    ...
                    <Card className="border-border/60 bg-transparent p-8">
                      <CardContent className="pt-0">
                        <div className="space-y-4">
                          ...
                          {service.features.map((feat) => (
                            <div key={feat} className="flex items-start gap-3 rounded-lg border border-border/60 bg-card p-4">
        ...
        <div className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8">
          ...
          <div className="mt-16 grid gap-8 md:grid-cols-5">
            {processSteps.map((step, i) => {
              ...
              return (
                <div key={step.step} className="reveal relative text-center" style={{ transitionDelay: `${i * 120}ms` }}>
      ...
      <section className="relative py-20 overflow-hidden">
        ...
        <div className="relative mx-auto max-w-4xl px-4 text-center sm:px-6 lg:px-8">
          ...
          <p className="mt-4 text-lg text-white/80 reveal">
            Let&apos;s build a practical roadmap that drives measurable growth.
          </p>

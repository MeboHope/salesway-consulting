...
export default function EditTeamMemberPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  const handleSubmit = async (e: React.FormEvent, publishStatus?: boolean) => {
    ...
    const { error: updateError } = await supabase.from('team_members').update(payload).eq('id', id);
    if (updateError) {
  ...
  if (loading) {
    return <div className="animate-pulse text-muted-foreground">Loading team member...</div>;
  }
  ...
  return (
    <div className="space-y-6 max-w-4xl">
      <div className="flex items-center gap-4">
        ...
        <h1 className="font-display text-2xl font-bold tracking-tight">Edit Team Member</h1>
      </div>
